				...
				//					cleardevice();
						...
						if(GetAsyncKeyState(VK_LBUTTON)==false && clicked == 1){
							...
							if(mousex()>getmaxx()/8 && mousey()>pegy-500 && mousex()<(getmaxx()/8)*3 && mousey()<getmaxy()){
								if(top[0]<n && picked>0){
									if(top[0]==-1 && picked_from!=0 || picked<ring[0][top[0]] && picked_from!=0){
										top[0]++;
										ring[0][top[0]]=picked;
										moves++;
									}
								...
								if(top[1]<n && picked>0){
									if(top[1]==-1 && picked_from!=1 || picked<ring[1][top[1]] && picked_from!=1){
										top[1]++;
										ring[1][top[1]]=picked;
										moves++;
									}
								...
								if(top[2]<n && picked>0){
									if(top[2]==-1 && picked_from!=2 || picked<ring[2][top[2]] && picked_from!=2){
										top[2]++;
										ring[2][top[2]]=picked;
										moves++;
									}
...
//								buff[0]="peg_candel_fire.jpg"
								readimagefile("table.jpg",0,0,getmaxx(),getmaxy());
								readimagefile("you_won.jpg",200,10,563,+10+259);
